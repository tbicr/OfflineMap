...
(function (window, mapbox, MM, initStorage) {
    initStorage(function (storage){
        var StorageRequestManager = function () {
            MM.RequestManager.apply(this, arguments);
        };
        ...
        StorageRequestManager.prototype.getLoadCompleteWithCache = function () {
            if (!this._loadComplete) {
                ...
                this._loadComplete = function(e) {
                    ...
                    if (storage) {
                        storage.add(this.id, theManager._imageToDataUri(this));
                    }
        ...
        var StorageTemplatedLayer = function(template, subdomains, name) {
            return new StorageLayer(new MM.Template(template, subdomains), null, name);
        };

        var map = mapbox.map('map');
        map.addLayer(new StorageTemplatedLayer('http://{S}.tile.osm.org/{Z}/{X}/{Y}.png', ['a', 'b', 'c']));
        map.ui.zoomer.add();
        map.ui.zoombox.add();
        map.centerzoom({lat: 53.902254, lon: 27.561850}, 13);
    });
})(window, mapbox, MM, initStorage);